...
class HierarchicalParticles(ParticlesOverlay):
  ...
  def all(self):
    """Get copy of complete particle set (isolated particles + subsystem constituents)"""
  ...
  def assign_parent_attributes(self, cset, pset, relative = True, recenter = True):
    """Make a parent from subsystem. Parent takes c.o.m of its childrens.
       Inputs:
       cset:    Children particle set
       pset:    Parent particle set"""
    pset.mass=cset.total_mass()
    if relative:
      pass
    else:
      pset.position = 0.*cset[0].position
      pset.velocity = 0.*cset[0].velocity
    if recenter:
      pset.position += cset.center_of_mass()
      pset.velocity += cset.center_of_mass_velocity()
      cset.move_to_center()
  
  def assign_subsystem(self, cset, parent, relative=True, recenter=True):
    """Assign children to their parent particle.
       Inputs:
       cset:    The children particle set"""
