...
class Nemesis(object):
  ...
  def sub_worker(self, cset, child_conv):
      ...
      smass = np.sort(cset.mass)
      """if len(cset)==2 and (len(cset[cset.type=="STAR"])>0
                or len(cset[cset.type=="BINARY STAR"])>0):
        dist = np.sqrt((cset[0].x-cset[1].x)**2
                      +(cset[0].y-cset[1].y)**2
                      +(cset[0].z-cset[1].z)**2)
        mass = cset.mass.sum()
        child_conv = nbody_system.nbody_to_si(mass, 2*dist)
        return (child_conv)"""

      if smass[-1]/smass[-2] > 500:
        return Mercury(child_conv)
        
      code = Hermite(child_conv)
  ...
  def evolve_model(self, tend, timestep=None):
    ...
    while self.model_time < (tend-timestep/2.): 
      ...
      self.kick_codes(timestep/2.)                  #Kind of long + energyloss
      self.drift_global(self.model_time+timestep, 
                        self.model_time+timestep/2) #Very long + energyloss
      self.drift_child(self.model_time+timestep)    #Kind of long
      self.kick_codes(timestep/2.)                  #Kind of long + energyloss
      self.split_subcodes()
      self.stellar_evolution(self.model_time+timestep/2.)
      self.star_channel_copier()
  ...
  def split_subcodes(self):
    ...
    for parent,subsys in list(subsystems.items()):
      ...
      if len(components) > 1:                  #Checking for dissolution of system
        ...
        for c in components:
          key_arr = np.concatenate((key_arr, c.key), axis=None)
          mass_arr = np.concatenate((mass_arr, np.sum(c.mass)), axis=None)
  ...
  def handle_supernova(self, SN_detect, bodies, time):
    ...
    for ci in range(len(SN_detect.particles(0))):
      ...
      self.event_key.append(SN_parti.key)
      self.event_mass.append(SN_parti.mass)
  ...
  def drift_child(self, dt):
    ...
    def evolve_model(system, dt):
      sc = system.stopping_conditions.collision_detection
      sc.enable()
      while system.model_time<dt*(1-1e-12):
        system.evolve_model(dt)
        if sc.is_set():
          print("detected", sc, system)
          E0a = self.energy_corr()
          coll_time = system.model_time
          for ci in range(len(sc.particles(0))):
            enc_particles_set = Particles(particles=[sc.particles(0)[ci],
                                                     sc.particles(1)[ci]])
            enc_parti = enc_particles_set.get_intersecting_subset_in(self.particles.all())
            
            for parents, children in self.particles.collection_attributes.subsystems.items():
              if enc_parti[0] in children:
                parent = parents
                parent_list.append(parent)
                child = children
                
            new_parti = Particles(1)
            new_parti.mass = np.sum(enc_parti.mass)
            new_parti.velocity = enc_parti.center_of_mass_velocity()
            new_parti.position = enc_parti.center_of_mass()
            new_parti.disk_key = max(enc_parti.disk_key)
            if len(enc_parti[enc_parti.type=="BINARY STAR"])!=0:
              new_parti.sub_worker_radius = self.psetup.ZAMS_radius(new_parti.mass)
              new_parti.type = "BINARY STAR"
            elif len(enc_parti[enc_parti.type=="STAR"])!=0:
              new_parti.radius = self.psetup.ZAMS_radius(new_parti.mass)
              new_parti.type = "STAR"
            else:
              new_parti.sub_worker_radius = np.sum(enc_parti.radius)
              new_parti.type = enc_parti[enc_parti.mass==enc_parti.mass.max()].type

            p = Particles()
            p.add_particle(new_parti)
            p.add_particle(system.particles)
            p.remove_particle(enc_particles_set)
            subcodes = self.subcodes
            if len(p)>1:
              newcode = self.subsys_code(p, self.child_conv)
              newcode.particles.add_particles(p)
              newparent = self.particles.add_subsystem(p)
              subcodes[newparent] = newcode
              ...
              max_mass_old = child[child.mass==child.mass.max()]
              idx = self.subsys_keys.index(max_mass_old.key)
              del self.chnl_from_sc_to_ccp[idx]
              del self.chnl_from_cc_to_all[idx]
              del self.subsys_keys[idx]

              idx = (newcode.particles.mass).argmax()
              self.subsys_keys.append(newcode.particles[idx].key)
              newparent.radius = self.psetup.parent_radius(np.sum(newcode.particles.mass), self.dt)

              self.particles.recenter_subsystems()
            
            else:
              if p.type=="BINARY STAR":
                p.type = "STAR"
              self.particles.remove_particle(parent)
              newparent = self.particles.add_subsystem(p)
              newparent.radius = self.psetup.parent_radius(p.mass, self.dt)

              chnl_temp = self.stellar_code.particles.new_channel_to(p,
                                                    attributes = ["radius", "mass"],
                                                    target_names = ["radius", "mass"])
              self.chnl_from_sc_to_ccp.append(chnl_temp)
              chnl_temp = p.new_channel_to(self.particles.all(),
                              attributes = ["radius", "mass"],
                              target_names = ["sub_worker_radius", "mass"])
              self.chnl_from_cc_to_all.append(chnl_temp)
              ...
              self.subsys_keys.append(newparent.key)
              newparent.radius = self.psetup.parent_radius(newparent.mass, self.dt)
          E1a = self.energy_corr()
          self.dEa+=(E1a-E0a)
    ...
    for sys_ in self.subcodes.values():
      if iter!=0:
        ...
        if offset>dt:
          print("curious?")
        threads.append(threading.Thread(target=evolve_model, 
                                        args=(sys_, dt-offset)))
      iter+=1
    if self.use_threading:
      for x in threads: 
        x.start()
      for x in threads: 
        x.join()  #Run local integration scheme
    else:
      for x in threads: 
        x.run()
    for par_ in parent_list:
      self.subcodes.pop(par_)
