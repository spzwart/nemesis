...
import nemesis_remote
import time
class CreateModel(object):
    
    
    def __init__(self, number_of_stars = 1024, number_of_solar_systems = 10, cluster_radius =  2 | units.parsec, seed = -1, number_of_planets = 10, minimum_radius = 100 | units.AU, maximum_radius = 400 | units.AU):
    ...
    def create_stars(self):
        mass = new_salpeter_mass_distribution(self.number_of_stars, random = self.rng)#, mass_min = 1 | units.MSun, mass_max = 1 | units.MSun)
        total_mass = mass.sum()
        converter = nbody_system.nbody_to_si(total_mass, self.cluster_radius)
        result = new_plummer_model(self.number_of_stars, converter, random = self.rng)
        result.mass = mass
...
def new_option_parser():
    ...
    result.add_option("--use-threading",
                  action="store_true", dest="use_threading", default=False,
                  help="if given use threads for the different evolve models")
...
class RunModelDistributed(RunModel):
    def create_code(self):
        result = nemesis_remote.DistributedNemesis(
            5
        )
    ...
    def run(self, end_time):
        ...
        t0 = time.time() | units.s
        self.code.recenter_systems()
        t1 = time.time() | units.s
        print("recentering the systems distributed DT:", t1-t0)
...
if __name__ == "__main__":
    ...
    if is_restart:
        ...
        uc =  CreateModel(o.ncluster,o.number_of_solar_systems, seed = o.seed, number_of_planets = o.nplanets, minimum_radius = o.minimum_radius, maximum_radius = o.maximum_radius)
        uc.start()
